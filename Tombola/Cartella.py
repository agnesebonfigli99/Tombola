...
class Cartella:
    ...
    def numero_estatto(self, i_riga, i_colonna):
        ...
        '''
        Metodo genera_cartella: il metodo genera_cartella si occupa di inserire in 4 posizioni randomiche per ogni riga il numero 1 per contrassegnare che quella casella rimarrà vuota
        ---> riferimento usato nella classe GruppoCartelle per il riempimento (nel riempimento effettivo della cartelle l'uno sarà sostituito con uno 0 da convenzione del programma)
        La cartella finora è una matrice di zeri.
        Contrassegniamo le posizioni della cartella che dovranno rimanere libere, per ogni riga (4), 
        inserendo in posizioni casuali il numero 1 --->si forza che su ogni riga ce ne siano 4 cioè le caselle da non riempire'''
